...
const TransactionDateItem = ({ date, amount, style }: Props) => {
  ...
  return (
    <View style={[{ flexDirection: "row", justifyContent: "space-between", alignItems: "flex-end"}, style]}>
      ...
      <Text style={[textStyle.title, { color: amount > 0 ? colors.income : colors.expense }]}>{`${currency.code} ${Math.abs(amount).toFixed(2)}`}</Text>
      </View>
    </View>
  );
};
